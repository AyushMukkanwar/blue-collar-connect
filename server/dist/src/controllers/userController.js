 ...
 */
export const createUserProfileById = async (c) => {
    return new Promise(async (resolve) => {
        ...
        bb.on("file", (fieldname, file, filename, encoding, mimeType) => {
            ...
            if (fieldname === "resume") {
                const isValidPDF = safeFilename.toLowerCase().endsWith(".pdf") ||
                    (mimeType &&
        ...
        bb.on("finish", async () => {
            ...
            try {
                ...
                resolve(c.json({ message: "Profile created successfully", profile: userProfile }, 201));
            }
            catch (error) {
                console.error("Error creating profile:", error);
                resolve(c.json({ error: error.message || "Error creating profile" }, 500));
            }
        });
        nodeReq.pipe(bb);
    });
 ...
 */
export const updateUserProfileById = async (c) => {
    return new Promise(async (resolve) => {
        const contentType = c.req.header("content-type");
        if (!contentType || !contentType.includes("multipart/form-data")) {
            resolve(c.json({ error: "Content-Type must be multipart/form-data" }, 400));
        ...
        bb.on("finish", async () => {
            ...
            try {
                ...
                if (files.profilePhoto && existingProfile?.profilePhoto) {
                    const oldPhotoUrl = existingProfile.profilePhoto;
                ...
                resolve(c.json({ error: error.message || "Error updating profile" }, 500));
            }
        });
        nodeReq.pipe(bb);
    });
 ...
 */
export const getUserProfileById = async (c) => {
    try {
        const userId = c.req.param("id");
        if (!userId) {
...
export const getUserRole = async (c) => {
    try {
