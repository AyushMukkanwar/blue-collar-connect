...
export const createJobPost = async (c) => {
        ...
        bb.on('finish', async () => {
            try {
                ...
                if (!employer_id || !job_title || !type_of_work) {
                    resolve(c.json({ error: 'Missing required fields: employer_id, job_title, and type_of_work are required' }, 400));
                    return;
                }
...
export const fetchJobPosts = async (c) => {
    try {
        ...
        let query = admin.firestore().collection('jobPosts');
...
export const fetchJobPostById = async (c) => {
    try {
