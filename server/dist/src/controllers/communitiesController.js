...
export const createWorkerCommunity = async (c) => {
    return new Promise(async (resolve) => {
        ...
        bb.on('field', (fieldname, val) => {
            if (fieldname === 'communityRules' || fieldname === 'communityTopics') {
                try {
                    ...
                    fields[fieldname] = val.split(',').map((item) => item.trim());
                }
            }
            else {
        ...
        bb.on('file', (fieldname, file, filename, encoding, mimeType) => {
            const safeFilename = typeof filename?.filename === 'string'
                ? filename.filename
            ...
            if (fieldname === 'communityProfilePhoto' || fieldname === 'communityBackgroundPhoto') {
                const isValidImage = (mimeType && validImageTypes.includes(mimeType.toLowerCase())) ||
...
export const joinWorkerCommunity = async (c) => {
    return new Promise(async (resolve) => {
        try {
...
export const leaveWorkerCommunity = async (c) => {
    return new Promise(async (resolve) => {
        try {
            ...
            bb.on('finish', async () => {
                ...
                const { userId, communityId, communityName } = fields;
                if (!userId || !communityId || !communityName) {
                    resolve(c.json({ error: 'Missing required fields: userId, communityId, and communityName are required' }, 400));
                    return;
                }
                // References to Firestore collections
                ...
                resolve(c.json({ message: 'Successfully left community (membership removed)' }, 200));
            });
            // Pipe the node stream into Busboy for processing
            nodeReq.pipe(bb);
        }
...
export const getCommunityById = async (c) => {
    return new Promise(async (resolve) => {
        try {
...
export const getUserCommunities = async (c) => {
    try {
...
export const searchCommunityByName = async (c) => {
    return new Promise(async (resolve) => {
        try {
            ...
            let results = querySnapshot.docs.map(doc => {
                const data = doc.data(); // Type assertion
                return {
                    communityId: doc.id,
            ...
            if (results.length === 0) {
                ...
                results = allCommunities.docs
                    ...
                    .filter(community => community.communityName.toLowerCase().includes(searchTerm))
                    .slice(0, limit);
            }
            resolve(c.json({
...
export const getAllCommunities = async (c) => {
    return new Promise(async (resolve) => {
        try {
...
export const createCommunityPost = async (c) => {
    return new Promise(async (resolve) => {
        const contentType = c.req.header('content-type');
        if (!contentType || !contentType.includes('multipart/form-data')) {
            resolve(c.json({ error: 'Content-Type must be multipart/form-data' }, 400));
...
//get communityPosts
export const getCommunityPosts = async (c) => {
    return new Promise(async (resolve) => {
        try {
...
export const getUserJoinedCommunityPosts = async (c) => {
    return new Promise(async (resolve) => {
        try {
            ...
            const communitiesWithPosts = await Promise.all(membershipDocs.map(async (membershipDoc) => {
                const membershipData = membershipDoc.data();
            ...
            resolve(c.json({ error: error.message || 'Error retrieving user joined community posts' }, 500));
        }
    });
};
export const addCommentToPost = async (c) => {
    return new Promise(async (resolve) => {
        try {
...
export const getCommunityPost = async (c) => {
    return new Promise(async (resolve) => {
        try {
